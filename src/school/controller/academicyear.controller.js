// academicyear.controller.js
import academicyearService from '../service/academicyear.service.js';
import dto from '../dto/academicyear.dto.js';

/**
 * Academicyear Controller
 * Expects req.user ({ id, name, email }) from auth middleware
 */

const parseBoolean = (val) => {
  if (typeof val === 'boolean') return val;
  if (!val && val !== false) return false;
  const v = String(val).toLowerCase();
  return v === 'true' || v === '1' || v === 'yes';
};

const createAcademicyear = async (req, res) => {
  try {
    const payload = req.body;
    if (req.user) {
      payload.created_by = req.user.id;
      payload.created_by_name = req.user.name;
      payload.created_by_email = req.user.email;
    }

    const ay = await academicyearService.createAcademicyear(payload);
    return res.status(201).json({ success: true, data: ay });
  } catch (err) {
    console.error('createAcademicyear error', err);
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
};

const updateAcademicyear = async (req, res) => {
  try {
    const { id } = req.params;
    const payload = req.body;
    if (req.user) {
      payload.updated_by = req.user.id;
      payload.updated_by_name = req.user.name;
      payload.updated_by_email = req.user.email;
    }

    const updated = await academicyearService.updateAcademicyear(id, payload);
    return res.status(200).json({ success: true, data: updated });
  } catch (err) {
    console.error('updateAcademicyear error', err);
    if (err.message && err.message.toLowerCase().includes('not found')) {
      return res.status(404).json({ success: false, message: err.message });
    }
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
};

const patchAcademicyear = async (req, res) => {
  // alias for partial updates
  return updateAcademicyear(req, res);
};

const getAcademicyears = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 10,
      search,
      startDate,
      endDate,
      order,
      includeAudit,
      includeDeleted,
      is_active,
      includeRelations, // NEW: optionally passed in query
    } = req.query;

    let filters = {};
    if (req.query.filters) {
      try {
        filters = typeof req.query.filters === 'object' ? req.query.filters : JSON.parse(req.query.filters);
      } catch (e) {
        return res.status(400).json({ success: false, message: 'Invalid filters JSON' });
      }
    }

    let parsedOrder;
    if (order) {
      try {
        parsedOrder = typeof order === 'object' ? order : JSON.parse(order);
      } catch (e) {
        if (typeof order === 'string' && order.includes(':')) {
          const [col, dir] = order.split(':');
          parsedOrder = [[col, dir.toUpperCase()]];
        }
      }
    }

    const opts = {
      page: Number(page) || 1,
      limit: Number(limit) || 10,
      filters,
      search,
      startDate,
      endDate,
      is_active: typeof is_active !== 'undefined' ? parseBoolean(is_active) : undefined,
      includeAudit: parseBoolean(includeAudit),
      includeDeleted: parseBoolean(includeDeleted),
      includeRelations: typeof includeRelations === 'undefined' ? true : parseBoolean(includeRelations), // default true
    };
    if (parsedOrder) opts.order = parsedOrder;

    const result = await academicyearService.getAcademicyears(opts);
    return res.status(200).json({ success: true, ...result });
  } catch (err) {
    console.error('getAcademicyears error', err);
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
};

const getAcademicyearById = async (req, res) => {
  try {
    const { id } = req.params;
    const includeDeleted = parseBoolean(req.query.includeDeleted);
    const includeAudit = typeof req.query.includeAudit === 'undefined' ? true : parseBoolean(req.query.includeAudit);
    const includeRelations = typeof req.query.includeRelations === 'undefined' ? true : parseBoolean(req.query.includeRelations);

    const ay = await academicyearService.getAcademicyearById(id, { includeDeleted, includeAudit, includeRelations });
    return res.status(200).json({ success: true, data: ay });
  } catch (err) {
    console.error('getAcademicyearById error', err);
    if (err.message && err.message.toLowerCase().includes('not found')) {
      return res.status(404).json({ success: false, message: err.message });
    }
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
};

const deleteAcademicyear = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedByMeta = {};
    if (req.user) {
      deletedByMeta.deleted_by = req.user.id;
      deletedByMeta.deleted_by_name = req.user.name;
      deletedByMeta.deleted_by_email = req.user.email;
    }
    await academicyearService.deleteAcademicyear(id, deletedByMeta);
    return res.status(200).json({ success: true, message: 'Academicyear deleted (soft)' });
  } catch (err) {
    console.error('deleteAcademicyear error', err);
    if (err.message && err.message.toLowerCase().includes('not found')) {
      return res.status(404).json({ success: false, message: err.message });
    }
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
};

const restoreAcademicyear = async (req, res) => {
  try {
    const { id } = req.params;
    const restoredByMeta = {};
    if (req.user) {
      restoredByMeta.updated_by = req.user.id;
      restoredByMeta.updated_by_name = req.user.name;
      restoredByMeta.updated_by_email = req.user.email;
    }
    const ay = await academicyearService.restoreAcademicyear(id, restoredByMeta);
    return res.status(200).json({ success: true, data: ay });
  } catch (err) {
    console.error('restoreAcademicyear error', err);
    if (err.message && err.message.toLowerCase().includes('not found')) {
      return res.status(404).json({ success: false, message: err.message });
    }
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
};

export default {
  createAcademicyear,
  updateAcademicyear,
  patchAcademicyear,
  getAcademicyears,
  getAcademicyearById,
  deleteAcademicyear,
  restoreAcademicyear,
};
